...
type
    TfrmGiftParam = class(TfrmModuleForm)
...
var
    frmGiftParam: TfrmGiftParam;

implementation

{$R *.dfm}

procedure TfrmGiftParam.FormCreate(Sender: TObject);
begin
    inherited;
...
procedure TfrmGiftParam.UniQuery_GiftParamCalcFields(DataSet: TDataSet);
begin
    inherited;
    DataSet.FieldByName('Index').AsInteger := UniQuery_GiftParam.RecNo;
end;

procedure TfrmGiftParam.UniQuery_GiftParamBeforeInsert(DataSet: TDataSet);
var
    i: Integer;
begin
    inherited;
...
procedure TfrmGiftParam.UniQuery_GiftParamBeforePost(DataSet: TDataSet);
begin
    inherited;
...
procedure TfrmGiftParam.UniQuery_GiftParamAfterScroll(DataSet: TDataSet);
begin
    inherited;
...
procedure TfrmGiftParam.UniQuery_GiftParamBeforeEdit(DataSet: TDataSet);
begin
    inherited;
...
procedure TfrmGiftParam.DS_DeleteExecute(Sender: TObject);
begin
  //inherited;
    if (TDataSet(UniQuery_GiftParam).FieldByName('status').AsInteger = 2) or
        (TDataSet(UniQuery_GiftParam).FieldByName('status').AsInteger = 3) then
