...
begin
  ...
  begin //机型不能为空，即使下拉框里面有选项 2014.2.21
    if Length(Trim(cbbCom_TestModel.Text)) > 0 then
    begin
      stem := '添加 自动测试计划#[' + '机型:' + Trim(cbbCom_TestModel.Text) + '/';
...
begin
  if Length(Trim(cbbCom_TestModel.Text)) > 0 then
  begin     //机型不能为空，即使下拉框里面有选项 2014.2.21
    stem := '删除 自动测试计划#[' + '机型:' + Trim(cbbCom_TestModel.Text) + '/';
    stem := stem + '删除可选测试项:' + trim(DBLDstList.SelectedItem);
    InsertOperRecord(User.UserName,stem + ']',DateTimeToStr(Now));
...
begin
  ...
  begin     //机型不能为空，即使下拉框里面有选项 2014.2.21
    ...
    begin
      stem := '添加 自动测试计划#[' + '机型:' + Trim(cbbCom_TestModel.Text) + '/';
      stem := stem + '添加可选测试项:' + trim(UniQuery_DstSubItem.FieldByName('ItemName').AsString);
      InsertOperRecord(User.UserName,stem + ']',DateTimeToStr(Now));
