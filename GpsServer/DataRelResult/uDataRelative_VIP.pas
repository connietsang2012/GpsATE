unit uDataRelative_VIP;

interface

uses
    Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
    Dialogs, DB, MemDS, DBAccess, Uni, StdCtrls, ComCtrls, GridsEh, DBGridEh, ComObj;

type
    TfrmDataRelative_VIP = class(TForm)
    ...
    UniQry_DataRelResultVIP: TWideStringField;
...
var
    frmDataRelative_VIP: TfrmDataRelative_VIP;
...
procedure TfrmDataRelative_VIP.Button1Click(Sender: TObject);
begin
    SearchTestResult;
    GroupBoxSearRes.Caption := ' 共' + IntToStr(UniQry_DataRelResult.RecordCount) + ' 记录';
end;

procedure TfrmDataRelative_VIP.SearchTestResult(sNowDate: string = '1');
var
    sTemp: string;
begin
    sSQL := 'SELECT *  FROM DataRelative_VIP';
    ...
    begin
        ...
        if Length(Trim(edt_VIP.Text)) > 0 then
            sWhere := sWhere + ' and (VIP like ''%' + Trim(edt_VIP.Text) + '%'') ';
...
procedure TfrmDataRelative_VIP.FormShow(Sender: TObject);
begin
    edt_IMEI.Text := '';
    edt_VIP.Text := '';
...
procedure TfrmDataRelative_VIP.DBGridEh1DblClick(Sender: TObject);
var
    sTp: string;
begin
    ...
    begin
        ...
        sTp := sTp + '       BAT：' + Trim(UniQry_DataRelResult.fieldbyname('VIP').AsString) + #10;

        Application.MessageBox(PChar(sTp), '服务号数据查询结果', MB_OK);
    end;
end;

procedure TfrmDataRelative_VIP.Button2Click(Sender: TObject);
begin
    DBGridExport(DBGridEh1);
end;

procedure TfrmDataRelative_VIP.DBGridExport(GRID: TDBGridEh);
