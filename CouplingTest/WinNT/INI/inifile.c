...
#define kMaxIniLineSize                 4096  //255  /* long lines should be split into multi-line entries */

#define kLineTag                        " Line"
#define INI_NUM_CHARS_PER_LINE          4096 //80
...
static int FindMultiLineItem(IniSection section, int startOfSearchIndex, int *startIndex, int *endIndex)
{
    ...
    for (index = startOfSearchIndex; index <= numItems; index++)
    {
        ...
        else if (lookingForEndOfMultiLine)
        {
...
static int ParseTagTextLine(IniText iniText, const char buf[], char *section, char *item, char *value)
{
    ...
    for (p=tagStart; FastTokenCmp(p, iniText->nameValueInputSeparatorToken); CmbStrInc(p))
        if (!*p)
...
//added by xingde
int  Ini_PutDouble_2Frac(IniText dest, const char *section, const char *itemName, double value)
{
    char buffer[256];

    sprintf(buffer, "%.2f", value);   /* JAG 8/13/96 PATCH 4.0.1 */
    return Ini_Put(dest, section, itemName, buffer, FALSE);
}
...
/****************Added By yintong****************************************/
int CVIFUNC Ini_PutString_Ex (IniText dest, const char *section, const char *itemName, const char *srcText)
{
    int     error = UIENoError;
    char    *escText = NULL;
    ...
    errChk( EscapeText(srcText, &escText));
    errChk( Ini_PutRawString_Ex(dest, section, itemName, escText));
...
int CVIFUNC Ini_PutRawString_Ex(IniText dest, const char *section, const char *itemName, const char *srcText)
{
    return Ini_PutSingleLineString_Ex (dest, section, itemName, srcText);  /* add quotes around string */
}

static int  Ini_PutSingleLineString_Ex (IniText theIniText, const char *section, const char *itemName, const char *value)
{
    int     error = UIENoError;
    ...
    errChk(Ini_Put(theIniText, section, itemName, value, FALSE));
