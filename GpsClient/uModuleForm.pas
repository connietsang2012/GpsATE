      ...
      begin
          ...
          begin
              ...
              begin
                  ...
                  else if (strPlanName='SMTIQCTest') and (SendCommRecord.sCmdKey<>'ATALLOWTEST') then
                  begin
...
begin
    ...
    else if CurrentFrom.ClassName='TfrmAutoTestMain' then
    begin
        TfrmAutoTestMain(CurrentFrom).SetResult(CommIndex,iResult);
        Exit;
    end
    else if CurrentFrom.ClassName='TfrmParamDownloadTest' then
    begin
        TfrmParamDownloadTest(CurrentFrom).SetResult(CommIndex,iResult);
        Exit;
    end
    else if CurrentFrom.ClassName='TfrmCoupleTest' then
    begin
        TfrmCoupleTest(CurrentFrom).SetResult(CommIndex,iResult);
        Exit;
    end
    else if CurrentFrom.ClassName='TfrmSMTIQCTest' then
    begin
...
begin
  ...
  if pos(#$A,StrCommRecText[CommIndex])<>0 then
    ...
    //读机子的IMEI
    else if strCmdCommKey[CommIndex]= 'ATEGMRR' then
    begin
    ...
    else if strCmdCommKey[CommIndex]= 'ATEGMRT' then
    begin
        if Pos('OK',StrCommRecText[CommIndex])>0 then
        begin
            SendToComm(CommIndex);
