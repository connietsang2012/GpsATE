...
var
    ...
    StrListCompare_AutoTestSMT: array[1..CommCount - 1] of TStringList; //比对（范围）        -1
    StrListCompare_Name_AutoTestSMT: array[1..CommCount - 1] of TStringList; //比对项目(名称)
    StrListCompareChar_AutoTestSMT: array[1..CommCount - 1] of TStringList; //比对（范围）
    StrListCompareChar_Name_AutoTestSMT: array[1..CommCount - 1] of TStringList; //比对项目(名称)
    LR_GPGSV_DataSMT: array[1..CommCount - 1] of TStringList;
    StrListMTKBMTSMT: array[1..CommCount - 1] of TStringList; //MTK电池电压通用指令返回值

    TempDBValue: array[1..CommCount - 1] of Integer;

    strCompareRf: string; //要比对的RF参数值
implementation
...
begin
    ...
    begin
        ...
        //各测试项参数
        StrListCompare_AutoTestSMT[iCommindex] := TStringList.Create; //比对（范围）
        StrListCompare_Name_AutoTestSMT[iCommindex] := TStringList.Create; //比对项目(名称)
        StrListCompareChar_AutoTestSMT[iCommindex] := TStringList.Create; //比对（范围）
    ...
    //Application.MessageBox(PChar(strAutoTestModel),'警告',MB_OK+MB_ICONWARNING);
    ...
    //////////unqry_TestItemParam_ByItemName_n 读取此机型数据库所有测试项//////////////
    //////////unqry_FULLDstSubItem  本地./GT02D 把服务器上的测试项一次性加载到本地//////
...
begin
    ...
    begin
        ...
        begin
            ...
            TestValue := 'DB值:' + inttostr(TempDBValue[CommIndex]);
        end;
